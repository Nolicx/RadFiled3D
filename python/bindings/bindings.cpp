...
#define DECLARE_SCALAR_VOXEL(m, dT, name, parent)                                        \
    py::class_<ScalarVoxel<dT>, std::shared_ptr<ScalarVoxel<dT>>, parent>(m, name)       \
        ...
        .def("set_data", [](ScalarVoxel<dT> &v, dT value) { v = value; })                \
        .def(py::self == py::self)                                                       \
        .def(py::self /= py::self)                                                       \
        .def(py::self *= py::self)                                                       \
        .def(py::self += py::self)                                                       \
        .def(py::self -= py::self)                                                       \
        .def("__repr__",                                                                 \
             [](const ScalarVoxel<dT> &a) { return "<RadFiled3D." + std::string(name) + " (" + std::to_string(a.get_data()) + ")>"; })
...
#define DECLARE_OWNING_SCALAR_VOXEL(m, dT, name, parent)                                       \
    py::class_<OwningScalarVoxel<dT>, std::shared_ptr<OwningScalarVoxel<dT>>, parent>(m, name) \
        ...
        .def("set_data", [](OwningScalarVoxel<dT> &v, dT value) { v.set_data(&value); })       \
        .def(py::self == py::self)                                                             \
        .def(py::self /= py::self)                                                             \
        .def(py::self *= py::self)                                                             \
        .def(py::self += py::self)                                                             \
        .def(py::self -= py::self)                                                             \
        .def("__repr__",                                                                       \
             [](const OwningScalarVoxel<dT> &a) { return "<RadFiled3D." + std::string(name) + " (" + std::to_string(a.get_data()) + ")>"; })
...
py::array create_owning_py_array(char *owned_buffer, size_t len, size_t element_size)
{
    ...
    py::capsule capsule(owned_buffer, [](void *p)
...
py::array owning_layer_array(std::shared_ptr<VoxelGrid> grid, bool channel_first)
{
    ...
    py::capsule capsule(data, [](void *p)
                        { delete[] static_cast<char *>(p); });
...
static py::array layer_to_owning_array(std::shared_ptr<VoxelGrid> grid, bool channel_first)
{
    ...
    {
    case Typing::DType::Double:
        return owning_layer_array<double>(grid, channel_first);
    case Typing::DType::Int:
        return owning_layer_array<int>(grid, channel_first);
    case Typing::DType::Char:
        return owning_layer_array<char>(grid, channel_first);
    case Typing::DType::Byte:
        return owning_layer_array<uint8_t>(grid, channel_first);
    case Typing::DType::UInt64:
        return owning_layer_array<uint64_t>(grid, channel_first);
    case Typing::DType::UInt32:
        return owning_layer_array<unsigned long>(grid, channel_first);
    default:
        return owning_layer_array<float>(grid, channel_first);
    }
}
...
PYBIND11_MODULE(RadFiled3D, m)
{
    ...
    py::class_<FiledTypes::V1::RadiationFieldMetadataHeader::Simulation::XRayTube>(m, "RadiationFieldXRayTubeMetadataV1")
        ...
        .def_property("tube_id", [](FiledTypes::V1::RadiationFieldMetadataHeader::Simulation::XRayTube &self) -> std::string
                      { return std::string(self.tube_id); }, [](FiledTypes::V1::RadiationFieldMetadataHeader::Simulation::XRayTube &self, const std::string &s)
    ...
    py::class_<FiledTypes::V1::RadiationFieldMetadataHeader::Simulation>(m, "RadiationFieldSimulationMetadataV1")
        ...
        .def_property("geometry", [](FiledTypes::V1::RadiationFieldMetadataHeader::Simulation &self) -> std::string
                      { return std::string(self.geometry); }, [](FiledTypes::V1::RadiationFieldMetadataHeader::Simulation &self, const std::string &s)
                      { strncpy(self.geometry, s.c_str(), std::min(sizeof(self.geometry), s.length())); })
        .def_property("physics_list", [](FiledTypes::V1::RadiationFieldMetadataHeader::Simulation &self) -> std::string
                      { return std::string(self.physics_list); }, [](FiledTypes::V1::RadiationFieldMetadataHeader::Simulation &self, const std::string &s)
        ...
        .def(py::pickle([](FiledTypes::V1::RadiationFieldMetadataHeader::Simulation &self)
                        { return MetadataHeaderSimulationPickleTuple(
                              ...
                              std::tuple(
                                  ...
                                  std::string(self.tube.tube_id))); }, [](const MetadataHeaderSimulationPickleTuple &t)
                        {
    ...
    py::class_<FiledTypes::V1::RadiationFieldMetadataHeader::Software>(m, "RadiationFieldSoftwareMetadataV1")
        .def(py::init([](const std::string &name, const std::string &version, const std::string &repository, const std::string &commit, const std::string &doi = "")
                      { return FiledTypes::V1::RadiationFieldMetadataHeader::Software(name, version, repository, commit, doi); }),
             py::arg("name"), py::arg("version"), py::arg("repository"), py::arg("commit"), py::arg("doi") = std::string(""))
        .def_property("name", [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self) -> std::string
                      { return std::string(self.name); }, [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self, const std::string &s)
                      { strncpy(self.name, s.c_str(), std::min(sizeof(self.name), s.length())); })
        .def_property("version", [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self) -> std::string
                      { return std::string(self.version); }, [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self, const std::string &s)
                      { strncpy(self.version, s.c_str(), std::min(sizeof(self.version), s.length())); })
        .def_property("repository", [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self) -> std::string
                      { return std::string(self.repository); }, [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self, const std::string &s)
                      { strncpy(self.repository, s.c_str(), std::min(sizeof(self.repository), s.length())); })
        .def_property("commit", [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self) -> std::string
                      { return std::string(self.commit); }, [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self, const std::string &s)
        ...
        .def(py::pickle([](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self)
                        { return MetadataHeaderSoftwarePickleTuple(
                              std::tuple(
                                  ...
                                  std::string(self.doi))); }, [](const MetadataHeaderSoftwarePickleTuple &t)
                        {
				...
				return software; }))
        .def_property("doi", [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self) -> std::string
                      { return std::string(self.doi); }, [](FiledTypes::V1::RadiationFieldMetadataHeader::Software &self, const std::string &s)
    ...
    py::class_<Storage::V1::RadiationFieldMetadata, std::shared_ptr<Storage::V1::RadiationFieldMetadata>, Storage::RadiationFieldMetadata>(m, "RadiationFieldMetadataV1")
        ...
        .def("get_dynamic_metadata", [](Storage::V1::RadiationFieldMetadata &self, const std::string &key)
            ...
            return VOXEL_REFERENCE(voxel); }, py::arg("key"), py::return_value_policy::reference)
        ...
        .def("add_dynamic_histogram_metadata", [](Storage::V1::RadiationFieldMetadata &self, const std::string &key, size_t bins, float bin_width)
			...
			IVoxel* voxel = self.get_dynamic_metadata().at(key);
            return VOXEL_REFERENCE(voxel); }, py::arg("key"), py::arg("bins"), py::arg("bin_width"), py::return_value_policy::reference)
        .def("add_dynamic_metadata", [](Storage::V1::RadiationFieldMetadata &self, const std::string &key, Typing::DType dtype)
            ...
            IVoxel* voxel = self.get_dynamic_metadata().at(key);
            return VOXEL_REFERENCE(voxel); }, py::arg("key"), py::arg("dtype"), py::return_value_policy::reference);
    ...
    py::class_<HistogramVoxel<float>, std::shared_ptr<HistogramVoxel<float>>, IVoxel>(m, "HistogramVoxel")
        ...
        .def(py::self == py::self)
        .def("__repr__", [](const HistogramVoxel<float> &a)
             {
    ...
    py::class_<OwningHistogramVoxel<float>, std::shared_ptr<OwningHistogramVoxel<float>>, HistogramVoxel<float>>(m, "OwningHistogramVoxel")
        ...
        .def(py::self == py::self)
        .def("__repr__", [](const OwningHistogramVoxel<float> &a)
             {
				const size_t bins = a.get_bins();
				const float bin_width = a.get_histogram_bin_width();
    ...
    py::class_<AngularResolvedVoxel<float>, std::shared_ptr<AngularResolvedVoxel<float>>, ScalarVoxel<float>>(m, "AngularResolvedVoxel")
        ...
        .def("get_value", [](AngularResolvedVoxel<float> &self, size_t phi_idx, size_t theta_idx)
             { return self.get_value(phi_idx, theta_idx); }, py::arg("phi_idx"), py::arg("theta_idx"), py::return_value_policy::reference)
        .def("get_value_by_coord", [](AngularResolvedVoxel<float> &self, float phi, float theta)
             { return self.get_value_by_coord(phi, theta); }, py::arg("phi"), py::arg("theta"), py::return_value_policy::reference)
        .def("add_value", [](AngularResolvedVoxel<float> &self, float phi, float theta, float value)
             { self.add_value(phi, theta, value); }, py::arg("phi"), py::arg("theta"), py::arg("value") = 1.f)
        .def("clear", &AngularResolvedVoxel<float>::clear)
        .def(py::self == py::self)
        .def("__repr__", [](const AngularResolvedVoxel<float> &a)
    ...
    py::class_<OwningAngularResolvedVoxel<float>, std::shared_ptr<OwningAngularResolvedVoxel<float>>, AngularResolvedVoxel<float>>(m, "OwningAngularResolvedVoxel")
        ...
        .def("add_value", [](OwningAngularResolvedVoxel<float> &self, float phi, float theta, float value)
             { self.add_value(phi, theta, value); }, py::arg("phi"), py::arg("theta"), py::arg("value") = 1.f)
        .def("clear", &OwningAngularResolvedVoxel<float>::clear)
        .def(py::self == py::self)
        .def("__repr__", [](const OwningAngularResolvedVoxel<float> &a)
    ...
    py::class_<VoxelBuffer, std::shared_ptr<VoxelBuffer>>(m, "VoxelBuffer")
        ...
        .def("add_histogram_layer", [](VoxelBuffer &self, const std::string &name, size_t bins, float bin_width, const std::string &unit)
             { self.add_custom_layer<HistogramVoxel<float>>(name, HistogramVoxel<float>(bins, bin_width, nullptr), 0.f, unit); }, py::arg("name"), py::arg("bins"), py::arg("bin_width"), py::arg("unit"))
        .def("add_spherical_layer", [](VoxelBuffer &self, const std::string &name, size_t phi_segments, size_t theta_segments, const std::string &unit)
             { self.add_custom_layer<AngularResolvedVoxel<float>>(name, AngularResolvedVoxel<float>(glm::uvec2(phi_segments, theta_segments), nullptr), 0.f, unit); }, py::arg("name"), py::arg("phi_segments"), py::arg("theta_segments"), py::arg("unit"));
    ...
    py::class_<VoxelGrid, std::shared_ptr<VoxelGrid>>(m, "VoxelGrid")
        ...
        .def("__enter__", [](std::shared_ptr<VoxelGrid> &self)
             { return self; })
    ...
    py::class_<PolarSegments, std::shared_ptr<PolarSegments>>(m, "PolarSegments")
        ...
        .def("__enter__", [](std::shared_ptr<PolarSegments> &self)
             { return self; })
        ...
        .def("get_layer_as_ndarray", [](std::shared_ptr<PolarSegmentsBuffer> &self, const std::string &layer, bool copy)
             {
				...
				return create_py_array_generic<float>(data, self->get_segments_count(), self, copy, element_size); }, py::arg("layer"), py::arg("copy") = false);
    ...
    py::class_<IRadiationField, std::shared_ptr<IRadiationField>>(m, "RadiationField")
        ...
        .def("__exit__", [&](std::shared_ptr<IRadiationField> &r, const std::optional<pybind11::type> &exc_type, const std::optional<pybind11::object> &exc_value, const std::optional<pybind11::object> &traceback)
             { r.reset(); }, "Exit the runtime context related to this object")
        .def("__enter__", [](std::shared_ptr<IRadiationField> &r)
             { return r; })
    ...
    py::class_<CartesianRadiationField, std::shared_ptr<CartesianRadiationField>, IRadiationField>(m, "CartesianRadiationField")
        ...
        .def("__exit__", [&](std::shared_ptr<CartesianRadiationField> &r, const std::optional<pybind11::type> &exc_type, const std::optional<pybind11::object> &exc_value, const std::optional<pybind11::object> &traceback)
             { r.reset(); }, "Exit the runtime context related to this object")
        .def("__enter__", [](std::shared_ptr<CartesianRadiationField> &r)
             { return r; })
        .def("__repr__", [](const CartesianRadiationField &a)
             {
    ...
    py::class_<PolarRadiationField, std::shared_ptr<PolarRadiationField>, IRadiationField>(m, "PolarRadiationField")
        ...
        .def("__exit__", [&](std::shared_ptr<PolarRadiationField> &r, const std::optional<pybind11::type> &exc_type, const std::optional<pybind11::object> &exc_value, const std::optional<pybind11::object> &traceback)
             { r.reset(); }, "Exit the runtime context related to this object")
        .def("__enter__", [](std::shared_ptr<PolarRadiationField> &r)
             { return r; })
        .def("__repr__", [](const PolarRadiationField &a)
             {
    ...
    py::class_<Storage::CartesianFieldAccessor, std::shared_ptr<CartesianFieldAccessor>, RadFiled3D::Storage::FieldAccessor>(m, "CartesianFieldAccessor")
        .def(py::init([](const std::shared_ptr<FieldAccessor> &base)
                      { return std::dynamic_pointer_cast<Storage::CartesianFieldAccessor>(base); }))
        ...
        .def("access_field_arrays", [](const Storage::CartesianFieldAccessor &self, const std::string &file, const std::vector<std::string> &channels, const std::vector<std::string> &layers, bool channel_first)
             {
                std::ifstream stream(file, std::ios::binary);
                return build_field_arrays(self, stream, channels, layers, channel_first); }, py::arg("file"), py::arg("channels"), py::arg("layers"), py::arg("channel_first") = true)
        .def("access_field_arrays_from_buffer", [](const Storage::CartesianFieldAccessor &self, const py::bytes &bytes, const std::vector<std::string> &channels, const std::vector<std::string> &layers, bool channel_first)
             {
                auto _bv = bytes_view(bytes); MemReadBuf _mb(_bv.first, _bv.second); std::istream stream(&_mb);
                return build_field_arrays(self, stream, channels, layers, channel_first); }, py::arg("buffer"), py::arg("channels"), py::arg("layers"), py::arg("channel_first") = true)
    ...
    py::class_<Storage::V1::CartesianFieldAccessor, std::shared_ptr<Storage::V1::CartesianFieldAccessor>, Storage::CartesianFieldAccessor>(m, "CartesianFieldAccessorV1")
        ...
        .def(py::init([](const std::shared_ptr<FieldAccessor> &base)
                      { return std::dynamic_pointer_cast<Storage::V1::CartesianFieldAccessor>(base); }))
    ...
    py::class_<Storage::FieldStore>(m, "FieldStore")
        ...
        .def_static("load_single_polar_layer_from_buffer", [](const std::string &bytes, const std::string &channel_name, const std::string &layer_name) -> std::shared_ptr<PolarSegments>
			    ...
			    return std::dynamic_pointer_cast<PolarFieldAccessor>(accessor)->accessLayer(stream, channel_name, layer_name); }, py::arg("bytes"), py::arg("channel_name"), py::arg("layer_name"));
        ...
        .def("get_as_ndarray", [](std::shared_ptr<VoxelCollection> &self, const std::string &channel, const std::string &layer, bool copy)
             {
                ...
                return create_owning_py_array<float>(data_buffer, voxel_count, element_size); }, py::arg("channel"), py::arg("layer"), py::arg("copy") = false)

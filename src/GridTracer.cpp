...
std::vector<size_t> DDAGridTracer::trace(const glm::vec3& p1, const glm::vec3& p2) {
	...
	// If the track has zero length, return an empty list of voxels
	// This can happen if p1 and p2 are the same point, which might occur due to numerical precision issues or if the track is very short
	if (trackLen == 0.0f) {
	...
	for (int i = 0; i < 3; ++i) {
		if (trackDir[i] == 0.0f) {
			trackDir[i] = 1e-30f; // Avoid division by zero in DDA calculations
		}
	}

    // Get grid information from the buffer
    glm::vec3 voxelDim = this->buffer.get_voxel_dimensions();
    glm::vec3 worldMin(0.0f); // RF3 implicitly adds world_min when using buffer information
    glm::ivec3 gridShape = glm::ivec3(this->buffer.get_voxel_counts());

    // Calculate starting and ending voxel indices for the track endpoints
    glm::ivec3 startVoxel = glm::floor((p1 - worldMin) / voxelDim);
    glm::ivec3 endVoxel = glm::floor((p2 - worldMin) / voxelDim);

    // DDA-Setup
	glm::ivec3 stepDir = glm::sign(trackDir);
    glm::vec3 nextVoxelBoundaryPosition = (
		(glm::vec3(startVoxel) + glm::vec3(stepDir.x > 0, stepDir.y > 0, stepDir.z > 0)) * voxelDim
	) + worldMin;

	// t as distance along the track to the next voxel boundary in each direction
    glm::vec3 tNextBoundary = (nextVoxelBoundaryPosition - p1) / trackDir;
    glm::vec3 tPerVoxel = glm::abs(voxelDim / trackDir);

	glm::ivec3 currentVoxel = startVoxel;
    while (true) {
        // Check if the current voxel is outside the grid bounds
        if (currentVoxel.x < 0 || currentVoxel.x >= gridShape.x  
            || currentVoxel.y < 0 || currentVoxel.y >= gridShape.y 
            || currentVoxel.z < 0 || currentVoxel.z >= gridShape.z) {
            break;
        }

		size_t voxelIdxFlat = this->buffer.get_voxel_idx(currentVoxel.x, currentVoxel.y, currentVoxel.z);
		voxels.push_back(voxelIdxFlat);

        // Determine which voxel boundary is closest and step to the next voxel in that direction
		int stepAxis =
			(tNextBoundary.x < tNextBoundary.y)
				? (tNextBoundary.x < tNextBoundary.z ? 0 : 2)
				: (tNextBoundary.y < tNextBoundary.z ? 1 : 2);

		// Calculate the distance to the next boundary along the track direction
        float tCurrent = tNextBoundary[stepAxis];
		// Check if maximum track length is exceeded before stepping to the next voxel
		if (tCurrent > trackLen) {
			break; 
		}
		// Check if stepping to the next boundary would exceed the track length
        if (currentVoxel.x == endVoxel.x
				&& currentVoxel.y == endVoxel.y
				&& currentVoxel.z == endVoxel.z
			) {
            break;
        }

        tNextBoundary[stepAxis] += tPerVoxel[stepAxis];
